...
// 		return true;
	    ...
	    else
	    {
	        ...
	        if(strpos($className,'\\')===false)  // class without namespace
	        {
	            ...
	            } else if (strpos($className, '_')) { // zend 方式寻找类路径
	                // Yii::app()->BasePath; // /opt/www/yii.com/protected 获取yii框架所在的根目录路径
	                $classPath = str_replace('_', '/', trim($className));
	                $classPath = trim($classPath, '/');
	                //$classPath = Yii::getPathOfAlias('ext') . '/' . $classPath . '.php'; //<!-- yii扩展类库所在目录
	                $classPath = YII_PATH . '/../' . $classPath . '.php';
	                //Yii::getPathOfAlias('system') // 通过别名获取路径
	                if (is_file($classPath)) {
	                    include_once $classPath; // TODO 不知道会有问题不
	                } else {
	                    return false;
	                }
	            } else {
